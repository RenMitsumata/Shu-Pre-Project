...
DeferredPolygon::DeferredPolygon()
{
}
...
void DeferredPolygon::Init()
{
	...
	shader = new Shader2D_PostEffect;
	shader->Init();
}

void DeferredPolygon::Draw()
{
...
void DeferredPolygon::SetDeferredTexture(ID3D11ShaderResourceView * srv)
{
	shader->SetTexture(srv);
}

void DeferredPolygon::SetSize(float width, float height, XMFLOAT2 screenPos)
{
	VERTEX_UI vertex[4];
	for (int i = 0; i < 4; i++) {
...
void DeferredPolygon::SetSize(float width, float height, XMFLOAT3 screenPos)
{
